...
ClassImp ( AliEmcalJetTask2 )

//________________________________________________________________________
AliEmcalJetTask2::AliEmcalJetTask2() :
    AliAnalysisTaskSE ( "AliEmcalJetTask2" ),
    fTracksName ( "Tracks" ),
...
//________________________________________________________________________
AliEmcalJetTask2::AliEmcalJetTask2 ( const char* name ) :
...
//________________________________________________________________________
void AliEmcalJetTask2::UserCreateOutputObjects()
    {
    // Create user objects.
...
//________________________________________________________________________
void AliEmcalJetTask2::UserExec ( Option_t* )
    {
...
//________________________________________________________________________
void AliEmcalJetTask2::Terminate ( Option_t* )
    {
    // Called once at the end of the analysis.
    }

//________________________________________________________________________
void AliEmcalJetTask2::FindJets()
    {
    // Find jets.
    if ( !fTracks && !fClus )
        {
    ...
    if ( ( fJetType & kR020Jet ) != 0 ) { fRadius = 0.2; }
    else
    if ( ( fJetType & kR030Jet ) != 0 ) { fRadius = 0.3; }
    else
    if ( ( fJetType & kR040Jet ) != 0 ) { fRadius = 0.4; }
    ...
    if ( ( fIsMcPart || ( ( fJetType & kFullJet ) != 0 ) || ( ( fJetType & kChargedJet ) != 0 ) ) && fTracks )
        ...
        for ( Int_t iTracks = 0; iTracks < Ntracks; ++iTracks )
            ...
            if ( ( eta < fEtaMin ) || ( eta > fEtaMax ) || ( phi < fPhiMin ) || ( phi > fPhiMax ) )
                { continue; }
    ...
    if ( ( ( ( fJetType & kFullJet ) != 0 ) || ( ( fJetType & kNeutralJet ) != 0 ) ) && fClus )
        ...
        for ( Int_t iClus = 0; iClus < Nclus; ++iClus )
            {
            AliVCluster* c = NULL;
            AliEmcalParticle* ep = NULL;
            Double_t cEta = 0, cPhi = 0, cPt = 0;
            Double_t cPx = 0, cPy = 0, cPz = 0;

            if ( fIsEmcPart )
                {
                ep = static_cast<AliEmcalParticle*> ( fClus->At ( iClus ) );
                if ( !ep ) { continue; }
            ...
            if ( ( cEta < fEtaMin ) || ( cEta > fEtaMax ) || ( cPhi < fPhiMin ) || ( cPhi > fPhiMax ) ) { continue; }

            // offset of 100 to skip ghost particles uid = -1
            AliDebug ( 2, Form ( "Cluster %d accepted (label = %d)", iClus, c->GetLabel() ) );

            Double_t sqr_e = cPx * cPx + cPy * cPy + cPz * cPz;
            if ( fPionMassClusters ) { sqr_e += 0.13957 * 0.13957 ; } //MV: dirty, need better solution

            Double_t e = TMath::Sqrt ( sqr_e );
    ...
    // sort jets according to jet pt (descending)
    std::stable_sort ( jets_incl.begin() , jets_incl.end() , sort_descend() );
...
#ifdef FASTJET_VERSION
        if ( fDoGenericSubtractionJetMass && ( n > ij ) )
            {
...
//________________________________________________________________________
Bool_t AliEmcalJetTask2::DoInit()
    {
    ...
    // get the event
    fEvent = InputEvent();
    if ( !fEvent ) { AliError ( Form ( "%s: Could not retrieve event! Returning", GetName() ) ); return kFALSE; }
    if ( ! ( fEvent->FindListObject ( fJetsSubName ) ) && fJetsSub ) { fEvent->AddObject ( fJetsSub ); }


    if (fTracksCont)
        { if (!GetTracksContainer()) return kFALSE; }
    else
        { if (!GetTracksEvent()) return kFALSE; }


    if (fClustersCont)
        { if (!GetClusterContainer()) return kFALSE; }
    else
        { if (!GetClustersEvent()) return kFALSE; }
    ...
    if ( !fRhoName.IsNull() && !fRhoParam ) // get rho from the event
        ...
        if ( !fRhoParam ) { AliError ( Form ( "%s: Could not retrieve rho %s!", GetName(), fRhoName.Data() ) ); return kFALSE; }
        }
    ...
    if ( !fRhomName.IsNull() && !fRhomParam ) // get rhom from the event
        ...
        if ( !fRhomParam ) { AliError ( Form ( "%s: Could not retrieve rho %s!", GetName(), fRhomName.Data() ) ); return kFALSE; }
        }
    ...
    if ( ! ( fEvent->FindListObject ( fJetsName ) ) )
        ...
        { AliError ( Form ( "%s: Object with name %s already in event! Returning", GetName(), fJetsName.Data() ) ); return kFALSE; }

    return kTRUE;
    }

//________________________________________________________________________
Bool_t AliEmcalJetTask2::GetTracksEvent()
    {
    AliAnalysisManager* am = AliAnalysisManager::GetAnalysisManager();

    if ( fTracksName.EqualTo("Tracks") ) { am->LoadBranch ( "Tracks" ); }

    if ( !fTracks && !fTracksName.IsNull() )
        {
        fTracks = dynamic_cast<TClonesArray*> ( fEvent->FindListObject ( fTracksName ) );

        if ( !fTracks ) { AliError ( Form ( "%s: Pointer to tracks %s == 0", GetName(), fTracksName.Data() ) ); return kFALSE; }
        }

    if ( fTracks )
        {
        TClass cls ( fTracks->GetClass()->GetName() );
        if ( cls.InheritsFrom ( "AliMCParticle" ) || cls.InheritsFrom ( "AliAODMCParticle" ) ) { fIsMcPart = 1; }
        }
...
//________________________________________________________________________
Bool_t AliEmcalJetTask2::GetClustersEvent()
    {
    AliAnalysisManager* am = AliAnalysisManager::GetAnalysisManager();

    if ( fCaloName.EqualTo("CaloClusters") ) { am->LoadBranch ( "CaloClusters" ); }

    if ( !fClus && !fCaloName.IsNull() )
        {
        fClus = dynamic_cast<TClonesArray*> ( fEvent->FindListObject ( fCaloName ) );

        if ( !fClus ) { AliError ( Form ( "%s: Pointer to clus %s == 0", GetName(), fCaloName.Data() ) ); return kFALSE; }
        }

    if ( fClus )
        {
        TClass cls ( fClus->GetClass()->GetName() );
        if ( cls.InheritsFrom ( "AliEmcalParticle" ) ) { fIsEmcPart = 1; }
        }
...
//________________________________________________________________________
Bool_t AliEmcalJetTask2::GetTracksContainer()
    {
    fTracks = fTracksCont->GetArray();
    if ( !fTracks ) { AliError ( Form ( "%s: Pointer to tracks %s == 0", GetName(), fTracksName.Data() ) ); return kFALSE; }
...
//________________________________________________________________________
Bool_t AliEmcalJetTask2::GetClustersContainer()
    {
    fClus = fClustersCont->GetArray();
    if ( !fClus ) { AliError ( Form ( "%s: Pointer to clus %s == 0", GetName(), fCaloName.Data() ) ); return kFALSE; }
...
Int_t AliEmcalJetTask2::GetIndexSub ( Double_t ptsub, std::vector<fastjet::PseudoJet>& constituentsunsub )
    {
...
//______________________________________________________________________________________
void  AliEmcalJetTask2::SetType ( Int_t t )
    {
    if ( IsLocked() ) return;
    switch (t)
        {
        case 0:
            fJetType |= kFullJet;    break;
        case 1:
            fJetType |= kChargedJet; break;
        case 2:
            fJetType |= kNeutralJet; break;
        default:
            fJetType |= kChargedJet;
            cout << "AliEmcalJetTask::SetType --> invalid jet type int; setting default kChargedJet";
        }
    } // for backward compatibility only

//______________________________________________________________________________________
void AliEmcalJetTask2::SelectCollisionCandidates ( UInt_t offlineTriggerMask = AliVEvent::kMB )
    {
    if ( !fIsPSelSet )
        {
        fIsPSelSet = kTRUE;
...
//______________________________________________________________________________________
void AliEmcalJetTask2::SelectPhysicalPrimaries ( Bool_t s )
    {
    if ( IsLocked() ) return;
