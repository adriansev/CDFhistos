...
void AddTasksFlavourJet(
   ...
   Printf("############## CORRECT PT JET BY MISSING DAUGHTERS WITH TYPE 2 AND 3 ####################");

   const Int_t    nTypes = 2;
   const Double_t aTypes[] = { 2,3 };
   const TString  sTypes[] = { "VettSum", "pt-scheme" };
   ...
   TString jetname;
   //jet reconstruction
   AliEmcalJetTask *taskFJ = AddTaskEmcalJet(sUsedTrks.Data(),sUsedClus.Data(),iJetAlgo,R,iJetType);
      
   taskFJ->SelectCollisionCandidates(uTriggerMask);
      
   Printf("Jet Finder with R = %f",R);
   jetname = taskFJ->GetName();
   
  for (Int_t i=0; i<nTypes; i++) {
    sTypes[i].Prepend(sText);
    AddMyAnalysis(jetname,R,iCandType,sCutFile,dJetPtCut,dJetAreaCut,acctype,bIsMC,bIsReco,bIsMap,sTypes[i],triggerOnLeadingJet,"PicoTracks",AliAnalysisTaskEmcal::kND, aTypes[i]);
  }
}
...
void AddMyAnalysis(TString& jetname, Double_t aRadius,
   ...
   AliAnalysisTaskEmcal::TriggerType emcalTrig=AliAnalysisTaskEmcal::kND,
   Int_t typeOutJetDaughCorrec=2,
   Int_t typeThnsp=1
