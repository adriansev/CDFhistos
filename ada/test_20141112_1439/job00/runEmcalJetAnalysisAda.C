...
#include "AddTaskLocalRho.C"              // $ALICE_ROOT/PWGJE/EMCALJetTasks/macros/AddTaskLocalRho.C
...
#include "AddTaskEmcalJetSpectra.C"       // $ALICE_ROOT/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSpectra.C
...
TString     kPluginAliRootVersion    = "vAN-20141109";
...
//==============================================================================
//      DEBUG
Int_t           debug              =  105 ; // kFatal = 0, kError, kWarning, kInfo, kDebug, kMaxType
...
Bool_t         doBkg               = kTRUE;          // background estimation with AliAnalysisTaskRho

UInt_t         pSel                = AliVEvent::kMB;  // used event selection for every task except for the analysis tasks

Int_t          jettype             = kFULLJETS;    // 0 --> AliEmcalJetTask::kFullJet; 1 --> AliEmcalJetTask::kChargedJet; 2 --> AliEmcalJetTask::kNeutralJet

// acceptance cuts on jets
TString        acceptance_type     = "EMCAL";         // TPC, EMCAL, kUSER
...
void runEmcalJetAnalysisAda (const char* analysis_mode = "local", const char* plugin_mode = "test", const char* input = "data.txt")
    {
...
//__________________________________________________________________________________
    // ###  Loading of custom task (script + plugin)  ###
    // compile and load the custom local task in local macro ; add the the task beside TaskEmcalJetSample instance
//     TString myTask = "AliAnalysisTaskEmcalJet_mycustomtask.cxx";
...
// ################# Now: Add jet finders+analyzers
    ...
    Double_t       radius          = 0.2;        // default: 0.4
    ...
    // NOTE !!! AddTaskEmcalJetSample.C does not support multiple radiuses as the taskname string does not contain radius information
    Double_t radius_list[] = {0.2}; // for each radius make a jetfinder

    jf = AddTaskEmcalJet( tracksName.Data(), clustersCorrName.Data(), algo, radius, jettype, minTrPt, minClPt, ghostArea, recombScheme, tag, minJetPt, selectPhysPrim, lockTask);
...
//#####################################################################################
    ...
    //     const char*    cutType     = "TPC";           // DEFAULT
        ...
        TString rhoName_bkg = "Rho";
        AliAnalysisTaskRho* rhotask = AddTaskRho (jetFinderTaskKT->GetName(), tracksName, clustersCorrName, rhoName_bkg.Data(), radius, acceptance_type.Data(),
...
//#####################################################################################
  gROOT->LoadMacro("$ALICE_ROOT/PWGJE/EMCALJetTasks/macros/AddTaskLocalRho.C");
...
//   Int_t      leadhadtype        = 0;
  ...
  UInt_t     fitType            = kCombined;   // fitting strategy
  TArrayI    *centralities      = 0x0;                                  // centrality binning for qa
  UInt_t     runMode            = kLocal;        // run mode
  if ( kAnalysisMode.EqualTo("grid") ) { runMode = kGrid; }

  //__________________________________________________________________________________
  AliAnalysisTaskLocalRho* lRho =lRho = AddTaskLocalRho( tracksName.Data(), "", jf->GetJetsName(), nrho, lrho, jf->GetRadius(), jetptcut, jetareacut, acceptance_type_i, leadhadtype,
                        name, fitOpts, fitType, centralities, runMode);
  lRho->SetDebugLevel(debug);


//#####################################################################################
    gROOT->LoadMacro("$ALICE_ROOT/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSample.C");
//     const char *ntracks            = "Tracks";         // replaced default value
//     const char *nclusters          = "CaloClusters";   // replaced default value
//     const char *njets              = "Jets";           // replaced default value -> taken from jet finder
//     const char *nrho               = "Rho";            // replaced default value -> taken from jet finder
    Int_t       nCentBins          = 1;
//     Double_t    jetradius          = 0.2;              // replaced default value -> taken from jet finder
//     Double_t    jetptcut           = 1;                // replaced default value
    Double_t    jetareacut         = 0.6;
//     const char *type               = "EMCAL";          // replaced default value
//     Int_t       leadhadtype        = 0;                // replaced default value -> already set up
     const char *taskname           = "AliAnalysisTaskEmcalJetSample";

  //__________________________________________________________________________________
  AliAnalysisTaskEmcalJetSample* anaTask = AddTaskEmcalJetSample( tracksName.Data() , clustersCorrName.Data(), jf->GetJetsName(), jf->GetRhoName(), nCentBins, jf->GetRadius(),
                                    jetptcut, jetareacut, acceptance_type.Data(), leadhadtype, taskname);
  anaTask->SetDebugLevel(debug);


//#####################################################################################
  gROOT->LoadMacro("$ALICE_ROOT/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSpectra.C");
  const char *outfilename    = "JetSpectraOutput.root";
...
//   const char *lrho           = "lrho";
  ...
  const Double_t minArea     = 0.001; // default 0.4;
//   const char *nTracks        = "PicoTracks";

  //__________________________________________________________________________________
  AliAnalysisTaskEmcalJetSpectra* spectraTask = AddTaskEmcalJetSpectra (outfilename,  jf->GetName(),  acceptance_type_i, nrho, lrho, minPhi, maxPhi,  minEta, maxEta,  minArea, tracksName.Data());
  spectraTask->SetDebugLevel(debug);
